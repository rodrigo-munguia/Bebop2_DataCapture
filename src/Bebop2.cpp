  ...
  : m_ipAddress(ipAddress), m_discoveryPort(discoveryPort)
{
  ...
  _usingFullNavdata = false; //RodrigoM
...
//----------- Rodrigo M
double vpRobotBebop2::getLatitude() const { return m_latitude; }
double vpRobotBebop2::getLongitude() const { return m_longitude; }
double vpRobotBebop2::getAltitude() const { return m_altitude; }
//--------------------
...
//********************************************************
//  RodrigoM
void vpRobotBebop2::StartNavDataCapture()
{
   // Create log files for data capture
    
    char dir_file_name[100];
    strcpy(dir_file_name,  StoreDir);  // 
    strcat(dir_file_name, "GPS_raw.txt");
    fileGPS.open (dir_file_name);
      
    char dir_file_name1[100];
    strcpy(dir_file_name1,  StoreDir);  // 
    strcat(dir_file_name1, "Altitude.txt");
    fileALT.open (dir_file_name1);
    
    char dir_file_name2[100];
    strcpy(dir_file_name2,  StoreDir);  // 
    strcat(dir_file_name2, "Attitude.txt");
    fileATT.open (dir_file_name2);

    char dir_file_name3[100];
    strcpy(dir_file_name3,  StoreDir);  // 
    strcat(dir_file_name3, "Speed.txt");
    fileSPD.open (dir_file_name3);
 

  strncpy(_navdata->StoreDir, StoreDir, 100); // SetStoreDir for Fullnavdata module
  _navdata->StartNavDataCapture();
  _navdata->Datacapture = true;  //   Start data capture from Fullnavdata module
...
//--------------------
void vpRobotBebop2::StopNavDataCapture()
{
   _navdata->Datacapture = false;
   _navdata->StopNavDataCapture();
...
//-----------------
void vpRobotBebop2::CaptureSingleFrame()
{

    CaptureSingleFrameFlag = true;

}



void vpRobotBebop2::StartFramesCapture()
{ 
  // Initialization of frame capture
  f_idx = 0;
  ...
  for(int i = 0; i < MaXnumImgs;++i) {
        ...
        Frame f;
        //f.time = 0;
        f.data.create(static_cast<unsigned int>(m_videoWidth), static_cast<unsigned int>(m_videoHeight), CV_8UC1);
   ...
   //char file_name[100];
    char dir_file_name[100];
    strcpy(dir_file_name,  StoreDir);  // Copy the directory name in dir_file_name
...
//------------------------------------------------
void vpRobotBebop2::StopFramesCapture()
{
   ...
   for(int i = 0; i < f_idx;++i)
   {
       ...
       //resize up
        ...
        resize(frames[i].data, resized_up, cv::Size(up_width, up_height), cv::INTER_LINEAR);
 
        //store frame in disk;
        cv::imwrite(dir_file_name, resized_up, compression_params); // write frame to disk   
        
        // add entry to file text
        fileCAM << file_name << "\n";
...
//-------------------------------------------------------------------------
void vpRobotBebop2::storeSingleFrame()
{
   ...
   if (m_videoDecodingStarted) {
    ...
    if (m_bgr_picture->data[0] != NULL) {
      ...
      vpImageConvert::BGRToGrey(m_bgr_picture->data[0], reinterpret_cast<unsigned char *>(I.bitmap), I.getWidth(),
                                I.getHeight());
      vpImageConvert::convert(I, cv_img_grey);
      ...
      //resize up
      resize(cv_img_grey, resized_up, cv::Size(up_width, up_height), cv::INTER_LINEAR);
      //store frame in disk;
      cv::imwrite(file_name, resized_up, compression_params); // write frame to disk    
...
{  
   ...
   if (m_videoDecodingStarted) {
    ...
    if (m_bgr_picture->data[0] != NULL) {
      ...
      cv_img_grey.copyTo(frames[f_idx].data);
      frames[f_idx].time.tv_sec = tpe.tv_sec;
      frames[f_idx].time.tv_nsec = tpe.tv_nsec;

      f_idx++;

      //cv::imshow("frame", cv_img_grey );
      //cv::waitKey(0); 
      
      
      //std::cout << StoreDir << std::endl;
...
void vpRobotBebop2::computeFrame(ARCONTROLLER_Frame_t *frame)
{
  ...
  if (CaptureSingleFrameFlag == true)
  {
      storeSingleFrame();
      CaptureSingleFrameFlag = false; // only save one frame
  }
...
//--------------------------------------------------------------------------------------------------------------
void vpRobotBebop2::cmdSpeedChange(ARCONTROLLER_DICTIONARY_ELEMENT_t *elementDictionary, vpRobotBebop2 *drone)
{
       ...
       clock_gettime(CLOCK_REALTIME, &tpe); // get the current time
        ...
        if(drone->DataCaptureActive == true)
        {
          long us;
          us = tpe.tv_nsec/1000;
          long sec = tpe.tv_sec - (tpe.tv_sec/10000)*10000;
          unsigned long time_in_micros = (1000000 * sec) + us;
          drone->fileSPD << time_in_micros <<"," << std::setprecision(10) << speedX  << "," << speedY  << "," << speedZ << "\n";
...
//---------------------------------------------------------------------------------------------------------------
void vpRobotBebop2::cmdPositionChange(ARCONTROLLER_DICTIONARY_ELEMENT_t *elementDictionary, vpRobotBebop2 *drone)
{
      struct timespec tpe;  // create struct timespec
       clock_gettime(CLOCK_REALTIME, &tpe); // get the current time
            ...
            if (drone->DataCaptureActive == true)
                {
                  ...
                  long sec = tpe.tv_sec - (tpe.tv_sec/10000)*10000;
                  unsigned long time_in_micros = (1000000 * sec) + us;
                  drone->fileGPS << time_in_micros <<"," << std::setprecision(10) << latitude  << "," << longitude  << "," << altitude << ","<< 0<< ","<<0<< ","<<0<<"\n";
...
void vpRobotBebop2::cmdAltitudeChange(ARCONTROLLER_DICTIONARY_ELEMENT_t *elementDictionary, vpRobotBebop2 *drone)
{       
       struct timespec tpe;  // create struct timespec
       clock_gettime(CLOCK_REALTIME, &tpe); // get the current time
        ...
        {   
            ...
            if (arg != NULL)
            {
                ...
                if (drone->DataCaptureActive == true)
                {
                  ...
                  long sec = tpe.tv_sec - (tpe.tv_sec/10000)*10000;
                  unsigned long time_in_micros = (1000000 * sec) + us;
                  drone->fileALT << time_in_micros <<"," << std::setprecision(6) << altitude  <<"\n"; 
...
void vpRobotBebop2::cmdAttitudeChange(ARCONTROLLER_DICTIONARY_ELEMENT_t *elementDictionary, vpRobotBebop2 *drone)
{       
            ...
            if (drone->DataCaptureActive == true)
                {
                  ...
                  long sec = tpe.tv_sec - (tpe.tv_sec/10000)*10000;
                  unsigned long time_in_micros = (1000000 * sec) + us;
                  drone->fileATT << time_in_micros <<"," << std::setprecision(6) << roll  << "," << pitch  << "," << yaw <<"\n";
...
void vpRobotBebop2::cmdSensorStateListChangedRcv(ARCONTROLLER_DICTIONARY_ELEMENT_t *elementDictionary,
                                              vpRobotBebop2 *drone)
{
    ARCONTROLLER_DICTIONARY_ARG_t *arg = NULL;
...
bool vpRobotBebop2::useFullNavdata() {
    
    const std::string _ip = "192.168.42.1"; // RodrigoM
    
    if(not _usingFullNavdata) {
        Eigen::initParallel();
        try{
            //_navdata->init(_ip, FULL_NAVDATA_DEFAULT_PORT);
            _navdata->init(_ip, 56789); //RodrigoM
        }
...
bool vpRobotBebop2::isUsingFullNavdata() const{
    return _navdata->receivedData();
}

Fullnavdata *vpRobotBebop2::getFullNavdata() const {
    return _navdata;
}
